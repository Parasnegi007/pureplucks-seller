...
async function populateCities() {
    try {
        const response = await fetch("http://localhost:5000/api/dashboard/cities");
...
async function loadOrders() {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/dashboard/all-orders`, {
        headers: {
  ...
  document.getElementById("updateStatusBtn").addEventListener("click", async () => {
    ...
    try {
        const res = await fetch(`http://localhost:5000/api/dashboard/order/${orderId}/status`, {
            method: "PATCH",
            headers: {
